...
bool ImGui::Renderingtab(const char* icon, bool selected)
{
    ...
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 9, bb.Min.y + 8), ImColor(255, 255, 255, int(255 * GetStyle().Alpha)), icon);

    if (selected)
    {
        /////////// ambi modified color for testing
        GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(255, 255, 255, int(255 * GetStyle().Alpha)), 5);
        ///////////
        GetWindowDrawList()->AddRect(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(41, 41, 41, int(255 * GetStyle().Alpha)), 5);

        GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 9, bb.Min.y + 8), ImColor(0, 0, 0, int(255 * it_filled->second)), icon);
        GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x - 11 , bb.Min.y + 8.75f), ImVec2(bb.Min.x - 7 * it_filled->second, bb.Max.y - 8.75f), ImColor(255, 255, 255, int(255 * it_filled->second)), 10.f, ImDrawCornerFlags_Right);
    }
...
bool ImGui::Renderingsubtab(const char* icon, bool selected)
{
    ...
    const ImRect bb(pos, ImVec2(pos.x + 45, pos.y + 35));
    ...
    ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(bb.Min.x + ImGui::GetFontSize() / 2, bb.Max.y - it_fill->second * 5), ImVec2(bb.Min.x + 35, bb.Max.y - 1), ImColor(255, 255, 255, int(255 * it_filled->second)), 3, ImDrawCornerFlags_Top);

    {
        // ambi modifed the code
...
bool ImGui::KeyBind(const char* label, KeyBind_t* k, ImGuiKeyBindFlags flags)
{
    ...
    window->DrawList->AddRect(ImVec2(frame_bb.Max.x - frame_w, frame_bb.Min.y), frame_bb.Max, ImColor(94, 159, 255, int(255 * it_hovered->second)), style.FrameRounding, 15, 1.6f);
...
bool ImGui::MenuChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags, float rounding)
{
    ...
    parent_window->DrawList->AddRectFilled(p, p + size_arg, ImColor(16, 16, 17, int(255 * fill)), rounding);
    parent_window->DrawList->AddRect(p, p + size_arg, ImColor(41, 41, 41, int(255 * fill)), rounding, 15, 1.6f);
...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    const ImVec2 text_pos(window->DC.CursorPos.x + 3, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(94, 159, 255, int(255 * it_hovered->second)), style.FrameRounding);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(pos + ImVec2(3, 0), pos + ImVec2(square_sz, square_sz) + ImVec2(3, 0));
    ...
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(15, 15, 15, int(255 * fill)), 4);
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImColor(41, 41, 41, int(255 * fill)), 4, 15, 1.6f);
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImColor(94, 159, 255, int(255 * it_hovered->second)), 4, 15, 1.6f);
    ...
    //ImGui::PushFont(LexendLight);
    {
        ...
        PushStyleColor(ImGuiCol_Text, ImColor(130, 130, 130, int(255 * fill)).operator ImVec4());
        RenderText(ImVec2(check_bb.Max.x + 3 + style.ItemInnerSpacing.x, check_bb.Min.y + 3), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = (GetWindowWidth() - style.WindowPadding.x * 2) - 5;
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(3, 0), window->DC.CursorPos + ImVec2(3, 0) + ImVec2(w, 20));
    ...
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(255, 255, 255, int(255 * fill)), style.FrameRounding, 15, 1.6f);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(255, 255, 255, int(255 * it_hovered->second)), style.FrameRounding, 15, 1.6f);

    //ImGui::PushFont(LexendLight);
    {
        PushStyleColor(ImGuiCol_Text, ImColor(255, 255, 255, int(255 * fill)).operator ImVec4());
        RenderArrow(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), popup_open ? ImGuiDir_Up : ImGuiDir_Down);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(3, 0), window->DC.CursorPos + ImVec2(w, 8));
    const ImRect total_bb(frame_bb.Min + ImVec2(3, 0), frame_bb.Max + ImVec2(3, 0) + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(13, 13, 14, int(255 * fill)), 100);

    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(55, 54, 57, int(255 * fill)), 100);

    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(max(frame_bb.Min.x, grab_bb.Max.x * slide + 2), frame_bb.Max.y), ImColor(255, 255, 255, int(255 * fill)), 100);
  
    window->DrawList->AddRectFilled(ImVec2(max(frame_bb.Min.x,( grab_bb.Max.x * slide - 8)), frame_bb.Min.y - 2),  ImVec2(max(frame_bb.Min.x, grab_bb.Max.x * slide + 4), frame_bb.Max.y + 2), ImColor(150, 150, 150, int(255 * fill * 1.5f)), 100);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // Render
    window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Min.x + 4 * it_active->second, bb.Max.y), ImColor(255, 255, 255, int(255 * it_active->second)), 10.f, ImDrawCornerFlags_Right);
